...
class DiscoveryEngine:
    ...
    async def get_next_song(
        ...
        try:
            ...
            try:
                ...
                log.error_cat(Category.DISCOVERY, "Failed to get recent history", error=str(e))
                recent_yt_ids = set()
            ...
            log.error_cat(Category.DISCOVERY, "Discovery failed", error=str(e), guild_id=guild_id)
            return None
    ...
    async def _strategy_similar(self, user_id: int, recent_yt_ids: set[str]) -> YTTrack | None:
        ...
        try:
            ...
            log.error_cat(Category.DISCOVERY, "Similar strategy failed", error=str(e), user_id=user_id)
            return None
        ...
        try:
            ...
            for artist_name, _ in random.sample(top_artists, min(3, len(top_artists))):
                try:
                    ...
                    log.debug_cat(Category.DISCOVERY, f"Artist lookup failed for {artist_name}", error=str(e))
                    continue
            
            return None
        except Exception as e:
            log.error_cat(Category.DISCOVERY, "Artist strategy failed", error=str(e), user_id=user_id)
            return None
        ...
        try:
            ...
            if not playlists:
                ...
                try:
                    ...
                    log.debug_cat(Category.DISCOVERY, "Fallback search failed", error=str(e))
                return None
            ...
            log.error_cat(Category.DISCOVERY, "Wildcard strategy failed", error=str(e))
            return None
