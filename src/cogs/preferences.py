...
class PreferencesCog(commands.Cog):
    ...
    async def show_preferences(self, interaction: discord.Interaction):
        ...
        try:
            ...
            log.error_cat(Category.PREFERENCE, "Failed to show preferences", error=str(e), user_id=interaction.user.id)
            await interaction.followup.send("❌ Failed to load preferences", ephemeral=True)
        ...
        try:
            ...
            if self.preferences:
                ...
                try:
                    ...
                    log.debug_cat(Category.PREFERENCE, "Failed to get genres", error=str(e))
                ...
                try:
                    ...
                    log.debug_cat(Category.PREFERENCE, "Failed to record reaction", error=str(e))
            ...
            log.error_cat(Category.PREFERENCE, "Like command failed", error=str(e), user_id=interaction.user.id)
            try:
                await interaction.response.send_message("❌ Failed to like song", ephemeral=True)
            except:
                pass
    ...
    async def dislike(self, interaction: discord.Interaction):
        ...
        try:
            ...
            log.error_cat(Category.PREFERENCE, "Dislike command failed", error=str(e), user_id=interaction.user.id)
            try:
                await interaction.response.send_message("❌ Failed to dislike song", ephemeral=True)
            except:
                pass
